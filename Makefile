...
wordcount:
	docker run --network hadoop --env-file ./hadoop.env --restart on-failure:5 ivcinform/hadoop-base:1.2.1-hadoop2.8.1-java8 hdfs dfs -mkdir -p /input/
	docker run --network hadoop --env-file ./hadoop.env --restart on-failure:5 ivcinform/hadoop-base:1.2.1-hadoop2.8.1-java8 hdfs dfs -copyFromLocal /opt/hadoop-2.8.1/README.txt /input/
	docker run --network hadoop --env-file ./hadoop.env --restart on-failure:5 ivcinform/hadoop-wordcount
	docker run --network hadoop --env-file ./hadoop.env --restart on-failure:5 ivcinform/hadoop-base:1.2.1-hadoop2.8.1-java8 hdfs dfs -cat /output/*
	docker run --network hadoop --env-file ./hadoop.env --restart on-failure:5 ivcinform/hadoop-base:1.2.1-hadoop2.8.1-java8 hdfs dfs -rm -r /output
